...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        if (help && option->value && !strcmp(option->olong, "--help")) {
            ...
            printf("%s\n", version);
            return 1;
        } else if (!strcmp(option->olong, "--benchmark")) {
            args->benchmark = option->value;
        ...
        } else if (!strcmp(option->olong, "--endnum")) {
            if (option->argument)
                args->endnum = option->argument;
        } else if (!strcmp(option->olong, "--startnum")) {
            if (option->argument)
                args->startnum = option->argument;
        } else if (!strcmp(option->olong, "--threads")) {
            if (option->argument)
                args->threads = option->argument;
        }
    }
    /* commands */
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    ...
    Option options[] = {
        {"-b", "--benchmark", 0, 0, NULL},
        {"-h", "--help", 0, 0, NULL},
        {"-v", "--verbose", 0, 0, NULL},
        {NULL, "--version", 0, 0, NULL},
        {"-e", "--endnum", 1, 0, NULL},
        {"-s", "--startnum", 1, 0, NULL},
        {NULL, "--threads", 1, 0, NULL}
    };
